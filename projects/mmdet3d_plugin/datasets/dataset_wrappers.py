...
class My_Resample_Dataset(object):
    ...
    def __init__(self, dataset, extra_fracs = [0., 0.2, 5., 0., 2., 0., 0., 0., 0., 0.,]):
        self.dataset = build_dataset(dataset)
        self.CLASSES = self.dataset.CLASSES
        self.cat2id = {name: i for i, name in enumerate(self.CLASSES)}
        self.extra_fracs = extra_fracs
    ...
    def _get_sample_indices(self):
        ...
        for idx in range(len(self.dataset)):
            ...
            #ADD unique here
            unique_sample_cat_ids = np.unique(sample_cat_ids)
            for cat_id in unique_sample_cat_ids:
        ...
        for cls_inds, extra_ratio in zip(list(class_sample_idxs.values()), self.extra_fracs):
