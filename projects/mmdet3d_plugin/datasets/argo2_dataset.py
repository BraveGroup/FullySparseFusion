...
try:
    from projects.mmdet3d_plugin.core.utils.argo2_utils import yaw_to_quat, LABEL_ATTR
except ImportError:
    yaw_to_quat = None
    LABEL_ATTR = None
...
class Argo2Dataset(KittiDataset):
    ...
    def format_results(self,
        ...
        for i in mmcv.track_iter_progress(range(num_samples)):
            ...
            track_uuid = None
            if len(out_i['labels_3d'].shape) == 2:
                cat_id = out_i['labels_3d'].squeeze(-1).int().numpy().tolist()
            else:
                cat_id = out_i['labels_3d'].int().numpy().tolist()
        ...
        if pklfile_prefix is not None:
            dir = os.path.dirname(pklfile_prefix)
    ...
    def evaluate(self,
        ...
        if pklfile_prefix is not None:
            dir_name = os.path.dirname(pklfile_prefix)
            json_file_path = os.path.join(dir_name, 'eval_results.json')
            json.dump(ap_dict, open(json_file_path, 'w'), indent=2)
        return ap_dict
