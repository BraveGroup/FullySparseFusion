...
class SparseClusterHeadV2(SparseClusterHead):
    ...
    def forward(self, feats, pts_xyz=None, pts_inds=None):
        ...
        for h in self.task_heads:
            ...
            if 'iou' in ret_dict:
                iou_logits = ret_dict['iou']
                iou_logits_list.append(iou_logits)
        ...
        if len(iou_logits_list) > 0:
            outs.update(
                iou_logits=iou_logits_list
            )
    ...
    def loss_single_task(
        ...
        targets = self.get_targets(num_task_classes, cluster_xyz, cluster_batch_idx, gt_bboxes_3d, gt_labels_3d, reg_preds, task_id)
    ...
    def get_targets(self,
        ...
        task_id_list = [task_id] * len(cluster_xyz_list)
        target_list_per_sample = multi_apply(self.get_targets_single, num_task_class_list, \
            cluster_xyz_list, gt_bboxes_3d, gt_labels_3d, reg_preds_list, task_id_list)
    ...
    def get_targets_single(self,
        ...
        if num_cluster == 0:
            ...
            self.task_info[str(task_id)] = dict(
                num_preds=torch.tensor(0, dtype=torch.float32, device=cluster_xyz.device),
                num_pos_preds=torch.tensor(0, dtype=torch.float32, device=cluster_xyz.device),
                num_gts=torch.tensor(int(gt_bboxes_3d.tensor.shape[0]), dtype=torch.float32, device=cluster_xyz.device),
                assigned_gts=torch.tensor(0, dtype=torch.float32, device=cluster_xyz.device),
            )
            if self.loss_iou is not None:
        ...
        if self.train_cfg.get('assign_by_dist', False):
            assign_result = self.assign_by_dist_single(cluster_xyz, gt_bboxes_3d, gt_labels_3d, task_id)
        else:
        ...
        self.task_info[str(task_id)] = dict(
            num_preds=torch.tensor(cluster_xyz.shape[0], dtype=torch.float32, device=cluster_xyz.device),
            num_pos_preds=torch.tensor(sample_result.pos_inds.shape[0], dtype=torch.float32, device=cluster_xyz.device),
            num_gts=torch.tensor(sample_result.num_gts, dtype=torch.float32, device=cluster_xyz.device),
            assigned_gts=torch.tensor(sample_result.pos_assigned_gt_inds.unique().shape[0], dtype=torch.float32, device=cluster_xyz.device),
        )
