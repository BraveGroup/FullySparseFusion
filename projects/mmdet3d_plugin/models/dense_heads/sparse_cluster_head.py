...
class SparseClusterHead(BaseModule):
    ...
    def __init__(self,
        ...
        self.sync_cls_avg_factor = False if train_cfg is None else train_cfg.get('sync_cls_avg_factor', True)
        self.as_rpn = as_rpn
    ...
    def get_iou_labels(self, reg_preds, cluster_xyz, gt_bboxes_3d, pos_inds):
        ...
        ious = bbox_overlaps_3d(bbox_preds[:, :7], gt_bboxes_3d[:, :7], mode='iou', coordinate='lidar') #[num_preds, num_gts]
        ious = ious.max(1)[0]
    ...
    def assign_single(self,
        ...
        enlarged_box = self.enlarge_gt_bboxes(LiDARInstance3DBoxes(gt_bboxes_3d.tensor[:, :7]))
    ...
    def assign_by_dist_single(self,
        ...
        if num_gts == 0 or num_cluster == 0:
            # No ground truth or cluster, return empty assignment
            self.print_info[f'assign_recall.{self.tasks[task_id]["class_names"]}'] = cluster_xyz.new_zeros(1)
            if num_gts == 0:
                # No ground truth, assign all to background
                assigned_gt_inds[:] = 0
            return AssignResult(
        ...
        max_dist_per_cls = self.train_cfg['max_dist'][task_id]
        ...
        assert matched_gt_labels.max().item() < max_dist_per_cls.size(0)
        ...
        self.print_info[f'assign_recall.{self.tasks[task_id]["class_names"]}'] = num_matched_gt / (num_gts_t + 1e-5)
        # end log
