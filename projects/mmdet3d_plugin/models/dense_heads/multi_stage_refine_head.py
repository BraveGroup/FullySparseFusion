...
class MultiStageRefineHead(SparseClusterHeadV2):
    ...
    def loss(
        ...
        for i in range(len(self.tasks)):
            losses_this_task = self.loss_single_task(
                ...
                old_cls_logits[0] if old_cls_logits is not None else None,
                old_reg_preds[0]  if old_reg_preds is not None else None,
            )
    ...
    def loss_single_task(
        ...
        #if use 2d-3d hybrid assign, make sure the order don't change in modify_gt_for_single_task function
