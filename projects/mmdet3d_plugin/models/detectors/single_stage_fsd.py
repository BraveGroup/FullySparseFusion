...
class VoteSegmentor(Base3DSegmentor):
    ...
    def forward_train(self,
                      ...
                      extract_feat_only=False,
                      ):
        if self.tanh_dims == []:
            points = [p.contiguous() for p in points]
        elif self.tanh_dims is not None:
            for p in points:
        ...
        if extract_feat_only:
            return (neck_out, pts_coors, points, labels, vote_targets, vote_mask)
    ...
    def simple_test(self, points, img_metas, gt_bboxes_3d=None, gt_labels_3d=None, extract_feat_only=False, rescale=False):
        if self.tanh_dims == []:
            points = [p.contiguous() for p in points]
        elif self.tanh_dims is not None:
            for p in points:
...
class SingleStageFSD(SingleStage3DDetector):
    ...
    def __init__(self,
        ...
        if self.cfg.get('disable_pretrain', False):
    ...
    def forward_train(self,
        ...
        if runtime_info is not None:
            # runtime_info is not None means vote_detector serve as super class of two_stage_fsd
        ...
        sampled_out = self.sample(dict_to_sample, dict_to_sample['vote_offsets'], gt_bboxes_3d, gt_labels_3d) # per cls list in sampled_out ## to extract the fg by the classification score
