...
def paint_obj_bbox_only(obj, mask, anno_list, obj_id, all_score_map, cam_id, score_thre, img_size,):
    obj_score = obj['obj_score'] #result[0][i][j][-1]
    obj_bbox = obj['obj_bbox'] #result[0][i][j][0:4]

    x1, y1, x2, y2 = obj_bbox
    x1, y1, x2, y2 = round(x1), round(y1), round(x2), round(y2)
    obj_mask = np.zeros(img_size, dtype=np.bool)
...
def get_instance_mask(obj_list, obj_id, cam_id, score_thre):
    #obj id: begin idx
    img_size = (900, 1600)
...
def save_result_format(result_list, info):
    """
    ...
    for cam_id, result_list_cam in enumerate(result_list):
        ...
        for cls_name in name_nusc:
            ...
            mask_cam, anno_list_cam, obj_id = get_instance_mask(obj_list, obj_id, cam_id, score_thre)
...
def save_data(model, args):
    ...
    for idx in tqdm(range(num_infos)):
        ...
        save_result_format(result_list, info)
